...
import { CHANGE_OVER_TOOLS } from "./change-over-tools";
...
export const TOOLS: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        siteLocationId: { type: "number" },
        hostType: { type: "string", enum: ["partner", "vendor"] },
        hostPartnerId: { type: "number", description: "Exact partner host chosen from this site context." },
        hostVendorId: { type: "number", description: "Exact assigned vendor host selected by the user." },
        phone: { type: "string" }, email: { type: "string" },
        platePhotoUrl: { type: "string" }, vehiclePhotoUrl: { type: "string" },
        latitude: { type: "number" },
        longitude: { type: "number" },
      },
      additionalProperties: false,
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ticketId: { type: "number" },
        latitude: { type: "number" }, longitude: { type: "number" }, endingMileage: { type: "number" },
        idempotencyKey: { type: "string" },
        confirmed: { type: "boolean" },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        screen: { type: "string" },
        id: { type: "number" },
        subjectType: { type: "string", enum: ["asset", "task", "meeting", "file", "document", "announcement", "message", "note", "form", "transcript"] },
        itemId: { type: "string", format: "uuid", description: "Exact authorized item UUID; use document for a managed file-library document ID and file for a legacy finalized file ID." },
        query: { type: "string" },
        start: { type: "string" },
        end: { type: "string" },
        types: { type: "array", items: { type: "string", enum: ["asset", "task", "meeting", "file", "announcement", "message", "note", "form", "transcript"] } },
      },
      required: ["screen"],
      additionalProperties: false,
    },
