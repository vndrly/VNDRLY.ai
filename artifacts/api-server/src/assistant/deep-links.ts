...
//      structured tool input ({ screen, id, token, step }) into a
//      concrete URL string the UI can navigate to.
...
export function buildDeepLink(
  input: { screen: string; id?: number; token?: string; step?: string },
): string | { error: string } {
  ...
  if (def.supportsStepQuery && input.step) {
    url = `${url}?step=${encodeURIComponent(input.step)}`;
  }
  return url;
}
