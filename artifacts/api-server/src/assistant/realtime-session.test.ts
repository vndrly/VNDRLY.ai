...
describe("AskV Realtime session", () => {
  ...
  it("creates GA client secrets with session audio and tool config", async () => {
    ...
    expect(JSON.parse(String(init?.body))).toMatchObject({
      session: {
        type: "realtime",
        model: "gpt-realtime-2",
        audio: { input: { format: { type: "audio/pcm", rate: 24000 }, transcription: { model: "gpt-4o-mini-transcribe", language: "en", prompt: expect.stringContaining("VNDRLY") }, noise_reduction: { type: "near_field" }, turn_detection: { type: "semantic_vad", eagerness: "medium", create_response: true, interrupt_response: true } }, output: { voice: "marin" } },
        tool_choice: "auto",
  ...
  it("creates server-mediated WebRTC calls with SDP and session config", async () => {
    ...
    expect(JSON.parse(String(body.get("session")))).toMatchObject({
      ...
      model: "gpt-realtime-2",
      instructions: "Keep it short.",
      audio: { input: { format: { type: "audio/pcm", rate: 24000 }, transcription: { model: "gpt-4o-mini-transcribe", language: "es", prompt: expect.stringContaining("VNDRLY") }, noise_reduction: { type: "near_field" }, turn_detection: { type: "semantic_vad", eagerness: "medium", create_response: true, interrupt_response: true } }, output: { voice: "marin" } },
      tool_choice: "auto",
      tools: [{ type: "function", name: "query_tickets" }],
    });
  });
