...
describe("deriveWorkHubCapabilities", () => {
  ...
  it("does not grant a gate supervisor vendor-wide scheduling authority", () => {
    const capabilities = deriveWorkHubCapabilities({
      session: {
        userId: 11,
        role: "field_employee",
    ...
    expect(capabilities).not.toContain("task.assign");
    expect(capabilities).not.toContain("shift.manage");
  });

  it("lets a gate supervisor schedule work only in a gate or site context", () => {
    const capabilities = deriveWorkHubCapabilities({
      session: {
        userId: 11,
        role: "field_employee",
