...
describe("collaboration access database boundaries", () => {
  ...
  it("denies private-channel read to a Crew owner who was not invited", async () => {
    state.results = [
      [{ kind: "private", crewId: "crew" }],
      [],
      [{ mode: "owner" }],
      [{ ownerOrgType: "vendor", ownerOrgId: 1 }],
      [{ userId: 7 }],
    ];
    expect(await collaborationChannelScope(7, "private")).toMatchObject({
      readable: false,
      manager: false,
    });
  });
  it("allows Crew owners to manage Crew-wide channels", async () => {
    state.results = [
      [{ kind: "crew", crewId: "crew" }],
      [],
      [{ mode: "owner" }],
      [{ ownerOrgType: "vendor", ownerOrgId: 1 }],
      [{ userId: 7 }],
    ];
    expect(await collaborationChannelScope(7, "public")).toMatchObject({
      readable: true,
      manager: true,
    });
  });
  it("revokes Crew access after the company membership is removed", async () => {
    ...
    expect(await collaborationChannelScope(7, "former-company")).toMatchObject({
      readable: false,
      manager: false,
    });
  });
  it("denies private chat additions through the generic membership API", async () => {
    ...
    await expect(
      assertCollaborationInvite(
        { id: "chat", ownerOrgType: "vendor", ownerOrgId: 1 },
        8,
      ),
  ...
  it("denies cross-company membership without explicit invitation consent", async () => {
    ...
    await expect(
      assertCollaborationInvite(
        { id: "shared", ownerOrgType: "vendor", ownerOrgId: 1 },
        8,
      ),
    ).rejects.toThrow("forbidden");
  });
