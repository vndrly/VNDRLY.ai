...
import type { WorkHubCapability, WorkHubContextRef, WorkHubOwner } from "@workspace/api-zod";
...
export class WorkHubAccessError extends Error {
  ...
  constructor(kind: "forbidden" | "not_found") {
    super(kind === "not_found" ? "Work Hub resource not found" : "Work Hub action forbidden");
    this.name = "WorkHubAccessError";
    this.status = kind === "not_found" ? 404 : 403;
    this.code = kind === "not_found" ? "work_hub.not_found" : "work_hub.forbidden";
  }
}
...
const PARTICIPANT_CAPABILITIES: WorkHubCapability[] = [
  "channel.read", "channel.write", "file.download",
];
...
export function deriveWorkHubCapabilities(input: WorkHubAccessInput): WorkHubCapability[] {
...
export function createWorkHubAccess(input: WorkHubAccessInput): WorkHubAccess {
  return Object.freeze({
    ...
    visibilityRevision: input.visibilityRevision ?? `${input.session.userId}:${input.owner.type}:${input.owner.id}`,
  });
}

export function requireWorkHubCapability(access: WorkHubAccess, capability: WorkHubCapability): void {
  if (!access.capabilities.has(capability)) throw new WorkHubAccessError("forbidden");
}
