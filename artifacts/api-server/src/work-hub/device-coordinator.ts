...
export interface DeviceCoordinatorStore {
  ...
  listOrganizationDevices(owner: WorkHubOwner): Promise<DeviceRecord[]>;
...
export const databaseDeviceCoordinatorStore: DeviceCoordinatorStore = {
  ...
  async listOrganizationDevices(owner) { return (await db.select().from(workHubDevicesTable).where(and(eq(workHubDevicesTable.ownerOrgType, owner.type), eq(workHubDevicesTable.ownerOrgId, owner.id))).orderBy(workHubDevicesTable.createdAt)).map(mapDevice); },
...
export function createDeviceCoordinator(store: DeviceCoordinatorStore, options: { now?: () => Date; fanOut?: (event: DurableUserEvent) => void } = {}) {
  ...
  return {
    ...
    async revokeOrganizationDevice(actor: DeviceActor, deviceId: string) { const device = await store.findDevice(deviceId); if (!device || device.owner.type !== actor.owner.type || device.owner.id !== actor.owner.id || device.revokedAt) throw new WorkHubDeviceError("work_hub.not_found", "Device not found"); const now = clock(); await store.clearConnections(device.id); const updated = await store.updateDevice(device.id, { revokedAt: now, updatedAt: now }); if (!updated) throw new WorkHubDeviceError("work_hub.not_found", "Device not found"); return updated; },
    async renameDevice(actor: DeviceActor, deviceId: string, friendlyName: string) { const device = await requireOwnedDevice(actor, deviceId); const updated = await store.updateDevice(device.id, { friendlyName: normalizeName(friendlyName), updatedAt: clock() }); if (!updated) throw new WorkHubDeviceError("work_hub.not_found", "Device not found"); return updated; },
    async listDevices(actor: DeviceActor) { return store.listDevices(actor); },
    async listOrganizationDevices(actor: DeviceActor) { return store.listOrganizationDevices(actor.owner); },
