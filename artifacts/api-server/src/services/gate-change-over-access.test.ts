...
it("keeps an inactive stop-work site readable only while its gate assignment remains current", async () => {
  ...
  const client = {
    query: async (sql: string, values: unknown[]) => {
      ...
      if (sql.startsWith("SELECT id, vendor_people_id, role FROM user_org_memberships"))
        return { rows: [{ id: 8, vendor_people_id: 70, role: "member" }], rowCount: 1 };
      if (sql.startsWith("SELECT r.role FROM vendor_person_operational_roles"))
...
it("lets a current vendor administrator use Gate Mode even when their legacy role is gatekeeper", async () => {
  const client = {
    query: async (sql: string) => {
      if (sql.startsWith("SELECT id, role, session_version"))
        return { rows: [{ id: 7, role: "vendor", session_version: 4 }], rowCount: 1 };
      if (sql.startsWith("SELECT id, name, partner_id"))
        return { rows: [{ id: 3, name: "Current site", partner_id: 22 }], rowCount: 1 };
      if (sql.startsWith("SELECT id FROM site_work_assignments"))
        return { rows: [{ id: 10 }], rowCount: 1 };
      if (sql.startsWith("SELECT id, vendor_people_id, role FROM user_org_memberships"))
        return { rows: [{ id: 8, vendor_people_id: 70, role: "admin" }], rowCount: 1 };
      throw new Error(`Unexpected query: ${sql}`);
    },
  };
  const session = {
  ...
  await expect(
    requireChangeOverAccess(client as never, session, 3),
