...
beforeEach(() => {
  // Each test owns new keys; existing counters are never cleared.
...
describe("PostgresBucketStore.increment", () => {
  it("opens a fresh window on first hit and increments within it", async () => {
    ...
    const a = await store.increment(`${namespace}:WIDGET`, "u:1", 10_000, t);
    expect(a.count).toBe(1);
    expect(a.resetAt).toBe(t + 10_000);

    const b = await store.increment(
      `${namespace}:WIDGET`,
      "u:1",
      10_000,
      t + 100,
  ...
  it("rolls the window once the previous reset time has passed", async () => {
    const store = new PostgresBucketStore();
    const t = Date.UTC(2026, 3, 27, 12, 0, 0);
    await store.increment(`${namespace}:WIDGET`, "u:1", 10_000, t);
    await store.increment(`${namespace}:WIDGET`, "u:1", 10_000, t + 100);

    // Cross the reset boundary — the next hit must start a brand-new
    // window with count = 1 and a fresh resetAt anchored at the new
    // `now`.
    const next = await store.increment(
      `${namespace}:WIDGET`,
      "u:1",
      10_000,
      t + 10_001,
  ...
  it("partitions counters per namespace even for the same key", async () => {
    const store = new PostgresBucketStore();
    const t = Date.UTC(2026, 3, 27, 12, 0, 0);
    const a = await store.increment(`${namespace}:ALPHA`, "u:1", 10_000, t);
    const b = await store.increment(`${namespace}:BETA`, "u:1", 10_000, t);
    expect(a.count).toBe(1);
    expect(b.count).toBe(1);
    const a2 = await store.increment(`${namespace}:ALPHA`, "u:1", 10_000, t);
    expect(a2.count).toBe(2);
    const b2 = await store.increment(`${namespace}:BETA`, "u:1", 10_000, t);
...
describe("PostgresBucketStore persistence (the property the task fixes)", () => {
  it("a second store instance reads the first instance's count", async () => {
    ...
    const a = await before.increment(
      `${namespace}:DAILY`,
      "2026-04-27",
      36 * 60 * 60 * 1000,
      t,
  ...
  it("two concurrent stores writing the same key agree on the count", async () => {
    ...
    const promises = [
      replicaA.increment(`${namespace}:IP`, "203.0.113.99", 10_000, t),
      replicaB.increment(`${namespace}:IP`, "203.0.113.99", 10_000, t),
      replicaA.increment(`${namespace}:IP`, "203.0.113.99", 10_000, t + 1),
      replicaB.increment(`${namespace}:IP`, "203.0.113.99", 10_000, t + 1),
      replicaA.increment(`${namespace}:IP`, "203.0.113.99", 10_000, t + 2),
    ];
...
describe("PostgresBucketStore.peek", () => {
  it("returns null when the bucket has never been written", async () => {
    const store = new PostgresBucketStore();
    const t = Date.UTC(2026, 3, 27, 12, 0, 0);
    expect(await store.peek(`${namespace}:WIDGET`, "missing", t)).toBeNull();
  });
  ...
  it("does not consume a hit", async () => {
    ...
    const store = new PostgresBucketStore();
    const t = Date.UTC(2026, 3, 27, 12, 0, 0);
    await store.increment(`${namespace}:DAILY`, "2026-04-27", 60_000, t);
    for (let i = 0; i < 5; i++)
      await store.peek(`${namespace}:DAILY`, "2026-04-27", t);
    const after = await store.increment(
      `${namespace}:DAILY`,
      "2026-04-27",
      60_000,
      t,
  ...
  it("hides elapsed windows", async () => {
    ...
    await store.increment(`${namespace}:WIDGET`, "u:1", 10_000, t);
    expect(
      await store.peek(`${namespace}:WIDGET`, "u:1", t + 10_001),
    ).toBeNull();
  });
});
...
describe("PostgresBucketStore.countActive", () => {
  it("counts only non-elapsed buckets in the namespace", async () => {
    const store = new PostgresBucketStore();
    const t = Date.UTC(2026, 3, 27, 12, 0, 0);
    await store.increment(`${namespace}:IP`, "1.1.1.1", 10_000, t);
    await store.increment(`${namespace}:IP`, "1.1.1.2", 10_000, t);
    // Stale row in the same namespace: opened earlier, already past
    // its reset by the time we call countActive.
    await store.increment(`${namespace}:IP`, "1.1.1.3", 1_000, t - 5_000);
    // Different-namespace row mustn't bleed in.
    await store.increment(`${namespace}:DAILY`, "2026-04-27", 10_000, t);

    const n = await store.countActive(`${namespace}:IP`, t);
    expect(n).toBe(2);
  });
});
...
describe("PostgresBucketStore.reset", () => {
  it("wipes only the requested namespace", async () => {
    const store = new PostgresBucketStore();
    const t = Date.UTC(2026, 3, 27, 12, 0, 0);
    await store.increment(`${namespace}:ALPHA`, "u:1", 10_000, t);
    await store.increment(`${namespace}:ALPHA`, "u:2", 10_000, t);
    await store.increment(`${namespace}:BETA`, "u:1", 10_000, t);
    await store.reset(`${namespace}:ALPHA`);
    expect(await store.countActive(`${namespace}:ALPHA`, t)).toBe(0);
    expect(await store.countActive(`${namespace}:BETA`, t)).toBe(1);
  });
});
