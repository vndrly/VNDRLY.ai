...
import { isSafetyIncidentInScope } from "../services/safety-incident-scope";
...
function sendError(res: Response, error: unknown) {
  if (error instanceof z.ZodError)
    return res
      .status(400)
      .json({ code: "safety.invalid_request", details: error.issues });
  const value = error as { status?: number; code?: string; message?: string };
  return res.status(value.status ?? 500).json({
    code: value.code ?? "safety.internal_error",
    message: value.message,
  });
}

async function fallbackAdmins(owner: {
  type: "vendor" | "partner";
  id: number;
}): Promise<number[]> {
...
async function loadResponse(
  ...
  const [[response], [event]] = await Promise.all([
    db
      .select()
      .from(safetyIncidentResponsesTable)
      .where(eq(safetyIncidentResponsesTable.eventId, eventId))
      .limit(1),
    db
      .select({
  ...
  try {
    ...
    if (!actor.owner)
      throw Object.assign(new Error("Organization context required"), {
        status: 403,
        code: "safety.owner_required",
      });
    const input = CreateSafetyIncidentInputSchema.parse(req.body);
    if (
      input.organizationId !== actor.owner.id &&
      actor.session.role !== "admin"
    ) {
      throw Object.assign(
        new Error("Incident is outside the active organization"),
        { status: 403, code: "safety.out_of_scope" },
      );
    }
    const [event] = await db
      ...
      .limit(1);
    if (!event)
      throw Object.assign(new Error("Safety event not found"), {
        status: 404,
        code: "safety.not_found",
      });
    assertEventAccess(actor, event);
    const [chain] = await db
      .select()
      .from(safetyEscalationChainsTable)
      .where(
        and(
      ...
      .limit(1);
    const recipients = chain?.responderUserIds.length
      ? chain.responderUserIds
      : await fallbackAdmins(actor.owner);
    const warning = chain?.responderUserIds.length
      ? null
      : "missing_safety_chain";
    const startedAt = input.startedAt ?? new Date();
    const [created] = await db
      .insert(safetyIncidentResponsesTable)
      .values({
        eventId: input.safetyEventId,
        source: input.source,
        severity: input.severity,
        responseStatus:
          input.source === "possible_crash" ? "awaiting_response" : "open",
        originalReport: input.originalReport,
        responseDeadlineAt:
          input.source === "possible_crash"
    ...
    if (
      input.source !== "possible_crash" &&
      recipients.length > 0 &&
      !input.degradedCapabilities.includes("push")
    ) {
...
router.post(
  "/implementation-a/safety/incidents/:eventId/escalate",
  async (req, res) => {
    try {
      const actor = context(req);
      ...
      if (response.acknowledgedAt || response.closedAt)
        return res.json(response);
      if (
        response.responseDeadlineAt &&
        new Date() < response.responseDeadlineAt
      ) {
        throw Object.assign(new Error("Response window is still active"), {
          status: 409,
          code: "safety.response_window_active",
        });
      }
      const [updated] = await db
        .update(safetyIncidentResponsesTable)
        .set({ responseStatus: "escalated", updatedAt: new Date() })
        .where(eq(safetyIncidentResponsesTable.id, response.id))
        .returning();
      if (updated.safetyChainSnapshot.length > 0) {
        await notifyUsers(updated.safetyChainSnapshot, {
          type: "safety_escalation",
          title: "Safety response required",
          body: "A possible crash received no response.",
          link: `/safety/${updated.eventId}`,
          category: "safety",
          dedupeKey: `safety_escalated:${updated.id}`,
        });
        await db.insert(safetyIncidentDeliveriesTable).values(
          updated.safetyChainSnapshot.map((recipientUserId) => ({
...
router.post(
  "/implementation-a/safety/incidents/:eventId/acknowledge",
  async (req, res) => {
    try {
      const actor = context(req);
      const response = await loadResponse(
      ...
      const [updated] = await db
        ...
        .set({
          ...
          assignedResponderUserId: actor.userId,
...
router.post(
  "/implementation-a/safety/incidents/:eventId/evidence",
  async (req, res) => {
    try {
      const actor = context(req);
      const response = await loadResponse(
      ...
      const [evidence] = await db
        .insert(safetyIncidentEvidenceTable)
        .values({
...
router.post(
  "/implementation-a/safety/incidents/:eventId/hold",
  async (req, res) => {
    try {
      const actor = context(req);
      const response = await loadResponse(
      ...
      const [hold] = await db
        .insert(safetyEvidenceHoldsTable)
        .values({
          responseId: response.id,
          reason,
...
router.post(
  "/implementation-a/safety/incidents/:eventId/close",
  async (req, res) => {
    try {
      const actor = context(req);
      const response = await loadResponse(
      ...
      if (!authorized)
        throw Object.assign(
          new Error(
            "Only the assigned responder, Safety Manager, or company admin may close this incident",
          ),
          { status: 403, code: "safety.close_forbidden" },
        );
      const now = new Date();
      await db.insert(safetyIncidentEvidenceTable).values({
        responseId: response.id,
        actorUserId: actor.userId,
        kind: "note",
        value: resolution,
