  ...
  try {
    ...
    if (!invitee)
      return sendApiError(
        res,
        404,
        "work_hub.invitee_not_found",
        "No VNDRLY user was found for that email",
      );
    const [member] = await db
      .insert(workHubChannelMembersTable)
      .values({ channelId: channel.id, userId: invitee.id, mode: "member" })
      .onConflictDoUpdate({
        target: [
          workHubChannelMembersTable.channelId,
          workHubChannelMembersTable.userId,
        ],
        set: { mode: "member" },
      })
      .returning();
...
router.get("/work-hub/channels/:channelId/messages", async (req, res) => {
  ...
  try {
    ...
    const rows = await db
      ...
      .where(
        and(
          eq(workHubMessagesTable.channelId, req.params.channelId),
          before && !Number.isNaN(before.getTime())
            ? lt(workHubMessagesTable.createdAt, before)
            : undefined,
        ),
      )
      .orderBy(
        desc(workHubMessagesTable.createdAt),
        desc(workHubMessagesTable.id),
      )
...
router.post("/work-hub/channels/:channelId/messages", async (req, res) => {
  ...
  try {
    ...
    const envelope = workHubCommandEnvelopeSchema.parse(req.body);
    const payload = createMessagePayload.parse(envelope.payload);
    if (
      envelope.owner.type !== channel.ownerOrgType ||
      envelope.owner.id !== channel.ownerOrgId
    )
    ...
    const result = await executeWorkHubCommand(
      ...
      async (tx) => {
        ...
        if (payload.mentionUserIds.length)
          await tx
            .insert(workHubMentionsTable)
            .values(
              [...new Set(payload.mentionUserIds)].map((mentionedUserId) => ({
    ...
    publishWorkHubEvent({
      ...
      owner: envelope.owner,
      context: envelope.context,
      subject: { type: "message", id: result.resource.id },
      recipientUserIds: payload.mentionUserIds,
    });
    return res.status(result.replayed ? 200 : 201).json(result);
  } catch (error) {
    return fail(res, error);
...
router.patch(
  "/work-hub/channels/:channelId/messages/:messageId",
  async (req, res) => {
    const actor = session(req);
    if (!actor)
      return sendApiError(
    ...
    try {
      ...
      const envelope = workHubCommandEnvelopeSchema.parse(req.body);
      const payload = updateMessagePayload.parse(envelope.payload);
      const result = await executeWorkHubCommand(
        { userId: actor.userId, source: source(req) },
        "message.update",
        envelope,
        async (tx) => {
          const [current] = await tx
            ...
            .where(
              and(
                eq(workHubMessagesTable.id, req.params.messageId),
                eq(workHubMessagesTable.channelId, channel.id),
              ),
            )
            .limit(1);
          if (!current) throw new WorkHubAccessError("not_found");
          if (current.authorUserId !== actor.userId && actor.role !== "admin")
            throw new WorkHubAccessError("forbidden");
          if (envelope.expectedVersion !== current.version)
            throw new Error("work_hub.version_conflict");
          await tx
            .insert(workHubMessageVersionsTable)
            .values({
          ...
          const [updated] = await tx
            .update(workHubMessagesTable)
            .set({
              body: payload.body,
              version: current.version + 1,
              editedAt: new Date(),
            })
            .where(eq(workHubMessagesTable.id, current.id))
            .returning();
          await appendWorkHubAudit(
            {
              actorUserId: actor.userId,
...
router.post(
  "/work-hub/channels/:channelId/messages/:messageId/reactions",
  async (req, res) => {
    const actor = session(req);
    if (!actor)
      return sendApiError(
    ...
    try {
      ...
      const result = await executeWorkHubCommand(
        ...
        async (tx) => {
          const where = and(
            eq(workHubReactionsTable.messageId, req.params.messageId),
            eq(workHubReactionsTable.userId, actor.userId),
            eq(workHubReactionsTable.emoji, payload.emoji),
          );
          const [existing] = await tx
            .select()
            .from(workHubReactionsTable)
            .where(where)
            .limit(1);
          if (existing) {
            await tx
              .delete(workHubReactionsTable)
              .where(eq(workHubReactionsTable.id, existing.id));
            return { active: false, emoji: payload.emoji };
          }
          await tx
            .insert(workHubReactionsTable)
            .values({
...
router.put("/work-hub/channels/:channelId/read-cursor", async (req, res) => {
  ...
  try {
    await resolveChannelAccess(actor, req.params.channelId, "channel.read");
    const lastMessageId = uuid
      .nullable()
      .parse(req.body?.lastMessageId ?? null);
    const [cursor] = await db
      .insert(workHubReadCursorsTable)
      .values({
      ...
      .onConflictDoUpdate({
        target: [
          workHubReadCursorsTable.channelId,
          workHubReadCursorsTable.userId,
        ],
        set: { lastMessageId, seenAt: new Date() },
      })
      .returning();
