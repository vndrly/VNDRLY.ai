...
import { createWorkHubAccess, requireWorkHubCapability, WorkHubAccessError } from "../work-hub/context-access";
import { listOwnedWorkHubChannels, resolveChannelAccess } from "../work-hub/queries";
...
function fail(res: Response, error: unknown): void {
  if (error instanceof WorkHubAccessError) { sendApiError(res, error.status, error.code, error.message); return; }
  if (error instanceof z.ZodError) { sendApiError(res, 400, "work_hub.invalid_operation", "Invalid Work Hub request", { issues: error.issues }); return; }
  if (error instanceof Error && error.message === "work_hub.version_conflict") { sendApiError(res, 409, "work_hub.version_conflict", "The item changed on another device"); return; }
  throw error;
}

router.use("/work-hub", async (_req, res, next) => {
  if (!(await isWorkHubEnabled())) { sendApiError(res, 404, "work_hub.not_found", "Not found"); return; }
  next();
});
...
router.post("/work-hub/channels/:channelId/messages", async (req, res) => {
  ...
  try {
    ...
    publishWorkHubEvent({ type: "work_hub.message.created", owner: envelope.owner, context: envelope.context, subject: { type: "message", id: result.resource.id }, recipientUserIds: payload.mentionUserIds });
    return res.status(result.replayed ? 200 : 201).json(result);
  } catch (error) { return fail(res, error); }
});
...
router.patch("/work-hub/channels/:channelId/messages/:messageId", async (req, res) => {
  ...
  try {
    ...
    const result = await executeWorkHubCommand({ userId: actor.userId, source: source(req) }, "message.update", envelope, async (tx) => {
      ...
      if (current.authorUserId !== actor.userId && actor.role !== "admin") throw new WorkHubAccessError("forbidden");
...
router.get("/work-hub/channels/:channelId/notes", async (req, res) => {
  const actor = session(req); if (!actor) return sendApiError(res, 401, "auth.unauthenticated", "Authentication required");
  try { await resolveChannelAccess(actor, req.params.channelId, "channel.read"); return res.json(await db.select().from(workHubNotesTable).where(eq(workHubNotesTable.channelId, req.params.channelId)).orderBy(desc(workHubNotesTable.updatedAt))); }
  catch (error) { return fail(res, error); }
});
...
router.post("/work-hub/channels/:channelId/notes", async (req, res) => {
  ...
  try {
    const { channel } = await resolveChannelAccess(actor, req.params.channelId, "channel.write");
    const envelope = workHubCommandEnvelopeSchema.parse(req.body); const payload = z.object({ title: z.string().trim().min(1).max(180), body: z.string().max(100_000).default("") }).parse(envelope.payload);
    const result = await executeWorkHubCommand({ userId: actor.userId, source: source(req) }, "note.create", envelope, async (tx) => {
      const [note] = await tx.insert(workHubNotesTable).values({ channelId: channel.id, title: payload.title, body: payload.body, createdById: actor.userId, updatedById: actor.userId }).returning(); return note;
    });
    return res.status(result.replayed ? 200 : 201).json(result);
  } catch (error) { return fail(res, error); }
});

router.patch("/work-hub/channels/:channelId/notes/:noteId", async (req, res) => {
  const actor = session(req); if (!actor) return sendApiError(res, 401, "auth.unauthenticated", "Authentication required");
  try {
    await resolveChannelAccess(actor, req.params.channelId, "channel.write");
    const envelope = workHubCommandEnvelopeSchema.parse(req.body); const payload = z.object({ title: z.string().trim().min(1).max(180), body: z.string().max(100_000) }).parse(envelope.payload);
    const result = await executeWorkHubCommand({ userId: actor.userId, source: source(req) }, "note.update", envelope, async (tx) => {
      const [current] = await tx.select().from(workHubNotesTable).where(and(eq(workHubNotesTable.id, req.params.noteId), eq(workHubNotesTable.channelId, req.params.channelId))).limit(1);
      if (!current) throw new WorkHubAccessError("not_found"); if (current.version !== envelope.expectedVersion) throw new Error("work_hub.version_conflict");
      await tx.insert(workHubNoteVersionsTable).values({ noteId: current.id, version: current.version, title: current.title, body: current.body, editorUserId: actor.userId });
      const [updated] = await tx.update(workHubNotesTable).set({ ...payload, version: current.version + 1, updatedById: actor.userId, updatedAt: new Date() }).where(eq(workHubNotesTable.id, current.id)).returning(); return updated;
    }); return res.json(result);
  } catch (error) { return fail(res, error); }
});
