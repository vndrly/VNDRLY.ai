...
function hasValidImageSignature(contentType: string, body: Buffer): boolean {
  ...
  if (type === "image/heic" || type === "image/heif")
    return body.subarray(4, 12).toString("ascii").includes("ftyp");
  return true;
}

async function canReadVisitEvidence(
  ...
  const [visit] = await db
    ...
    .where(
      or(
        eq(siteVisitsTable.platePhotoUrl, objectPath),
        eq(siteVisitsTable.vehiclePhotoUrl, objectPath),
      ),
    )
    .limit(1);
...
router.put(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!hasValidImageSignature(contentType, body)) {
      res
        .status(415)
        .json({ error: "Uploaded bytes do not match a supported image type" });
      return;
    }
    try {
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      const normalized =
        objectStorageService.normalizeObjectEntityPath(objectURL);
      if (!/^\/objects\/uploads\/[0-9a-f-]{36}$/i.test(normalized)) {
        res
          .status(403)
          .json({ error: "This object is managed by its owning feature" });
        return;
      }
      const [managedFile] = await db
        .select({ id: workHubFilesTable.id })
        .from(workHubFilesTable)
        .where(eq(workHubFilesTable.storageKey, normalized))
        .limit(1);
      if (managedFile) {
        res
          .status(403)
          .json({ error: "Work Hub files must be finalized through Work Hub" });
        return;
      }
      const existingAcl =
        await objectStorageService.getStoredObjectAcl(normalized);
      if (existingAcl?.owner && existingAcl.owner !== String(session.userId)) {
        res
          .status(403)
          .json({ error: "Cannot change another owner's object permissions" });
        return;
      }
      const objectPath = await objectStorageService.trySetObjectEntityAclPolicy(
        objectURL,
        {
          ...
          visibility,
          ...(req.body?.purpose === "gate-evidence"
            ? { purpose: "gate-evidence" as const }
            : {}),
        },
      );
      res.json({ objectPath });
...
router.delete("/storage/uploads", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [workHubReference] = await db
      .select({ id: workHubFilesTable.id })
      .from(workHubFilesTable)
      .where(eq(workHubFilesTable.storageKey, objectPath))
      .limit(1);
    if (workHubReference) {
      res
        .status(409)
        .json({
          error: "Object is attached to Work Hub; use its recycle controls",
        });
      return;
    }
    ...
    const [ticketReference] = await db
      .select({ id: ticketNoteLogsTable.id })
      .from(ticketNoteLogsTable)
      .where(ticketAttachmentReference(objectPath))
      .limit(1);
    if (ticketReference) {
