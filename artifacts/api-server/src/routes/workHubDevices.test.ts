...
const fenceAudioMock = vi.hoisted(() => vi.fn());
...
vi.mock("../work-hub/audio-lease-database", () => ({ fenceAudioLeasesForDevice: fenceAudioMock }));
...
describe("Work Hub device routes", () => {
  beforeEach(() => {
    ...
    preferencesMock.get.mockResolvedValue({ rankedDeviceIds: [], automaticBackupDeviceIds: [], learning: {} }); preferencesMock.save.mockImplementation(async (_actor, value) => value); auditMock.mockResolvedValue(undefined); fenceAudioMock.mockResolvedValue(undefined);
  });
  ...
  it("heartbeats and revokes only validated device identifiers", async () => {
    ...
    expect(fenceAudioMock).toHaveBeenCalledWith(deviceId);
  });
