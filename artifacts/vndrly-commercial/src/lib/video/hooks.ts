...
export interface UseVideoPlayerOptions {
  ...
  loop?: boolean;
  autoStart?: boolean;
  /** Scene index to start playback from (used for chunked capture). */
  startScene?: number;
}
...
export function useVideoPlayer(options: UseVideoPlayerOptions): UseVideoPlayerReturn {
  const { durations, onVideoEnd, loop = true, autoStart = true, startScene = 0 } = options;
  ...
  const [currentScene, setCurrentScene] = useState(
    Math.min(Math.max(0, startScene), totalScenes - 1),
  );
