...
export default function WorkHubScreen() {
  ...
  return <ScreenSafeArea style={{ backgroundColor: colors.background }}><ScrollView contentContainerStyle={{ padding: 20, gap: 16 }}>
    ...
    <View style={{ flexDirection: width >= 768 ? "row" : "column", flexWrap: "wrap", gap: 12 }}>
      ...
      <SummaryCard title="Chat" icon="message-circle" value="Open conversations" detail={`Messages, invitations, and recent ${membership?.orgName?.trim() || "company"} threads.`} onPress={() => openModule("chat")} width={cardWidth} />
      ...
      <SummaryCard title="Calls & Meetings" icon="phone-call" value={`${meetings} upcoming`} detail="Recent calls, meeting invitations, and catch-up information." onPress={() => openModule("meetings")} width={cardWidth} />
