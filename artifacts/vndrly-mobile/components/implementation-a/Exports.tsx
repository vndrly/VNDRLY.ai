...
export function ImplementationAExports({ owner }: { owner: { type: "vendor" | "partner"; id: number } }) {
  const colors = useColors();
  const [dataset, setDataset] = useState<Dataset>("payroll");
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const detail = OPTIONS[dataset];
  const download = async () => {
    setBusy(true); setError("");
    try {
      const token = await getToken();
      if (!token) throw new Error("Sign in again to create this export.");
      const response = await fetch(`${getApiBase()}/api/work-hub/exports/implementation-a`, { method: "POST", headers: { Authorization: `Bearer ${token}`, "content-type": "application/json", "x-vndrly-client": "ios" }, body: JSON.stringify({ dataset, scope: { ownerOrgType: owner.type, ownerOrgId: owner.id } }) });
      if (!response.ok) throw new Error("The export could not be created.");
      const uri = `${FileSystem.cacheDirectory}vndrly-${dataset}.csv`;
  ...
  return <ImplementationASurface description="Preview the company scope before creating an audited CSV.">
    <View accessibilityRole="radiogroup" style={{ gap: 8 }}>{(Object.keys(OPTIONS) as Dataset[]).map((value) => <Pressable key={value} accessibilityRole="radio" accessibilityState={{ checked: dataset === value }} onPress={() => setDataset(value)} style={{ borderWidth: 1, borderColor: dataset === value ? colors.primary : colors.border, borderRadius: 10, padding: 12 }}><Text style={{ color: colors.text, fontWeight: "700" }}>{OPTIONS[value][0]}</Text></Pressable>)}</View>
