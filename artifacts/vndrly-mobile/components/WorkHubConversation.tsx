...
export default function WorkHubConversation({ channel, onClose }: { channel: Record<string, any>; onClose: () => void }) {
  ...
  const send = async () => {
    ...
    try { await apiFetch(`${base}/messages${editing ? `/${editing.id}` : ""}`, { method: editing ? "PATCH" : "POST", body: JSON.stringify(envelope(JSON.parse(body), editing?.version, attempt.current.id)) }); changeDraft(""); setReply(null); setEditing(null); attempt.current = null; await load(); }
