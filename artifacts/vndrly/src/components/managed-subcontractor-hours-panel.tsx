...
export default function ManagedSubcontractorHoursPanel({ vendorId, companies, initialCompanyId, start, end, showSettings = false }: { vendorId: number; companies: Company[]; initialCompanyId?: string; start: string; end: string; showSettings?: boolean }) {
  ...
  return <section className="rounded-xl border bg-card p-4 space-y-4" data-testid="managed-subcontractor-hours-panel">
    <div className="flex flex-wrap items-start justify-between gap-3">
      ...
      {companies.length > 1 && <select aria-label="Subcontractor" className="rounded border bg-background px-3 py-2" value={companyId} onChange={(event) => setCompanyId(event.target.value)}>{companies.map((company) => <option key={company.id} value={company.id}>{company.name}</option>)}</select>}
    </div>
    ...
    {report.data && <>
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">Scheduled</span><strong className="block text-xl">{hours(report.data.totals.scheduledMinutes)} hrs</strong></div>
        <div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">Accrued</span><strong className="block text-xl">{hours(report.data.totals.actualMinutes)} hrs</strong></div>
        <div className="rounded-lg border p-3"><span className="text-xs text-muted-foreground">Approved</span><strong className="block text-xl">{hours(report.data.totals.approvedMinutes)} hrs</strong></div>
      </div>
      ...
      {showSettings && <div className="grid gap-3 rounded-lg border p-3 md:grid-cols-[12rem_1fr_auto]">
        ...
        <label className="text-sm">Email recipients<input className="mt-1 w-full rounded border bg-background p-2" value={recipients} onChange={(event) => setRecipients(event.target.value)} placeholder="payroll@subcontractor.com" /></label>
