...
export default function EmployeePortalLoginFields({
  ...
  }, [loginInfo?.hasLogin, loginInfo?.mustChangePassword, employeeId]);
  ...
  const signInPath =
    vendorRole === "foreman" || vendorRole === "both" ? "/foreman" : "/field";

  const saveCredentials = async () => {
    if (!credEmail || credPassword.length < 8) {
  ...
  const body = (
    <div className="space-y-4">
      <div
        className={`rounded-md border-2 p-3 flex items-start gap-2 ${
          loginInfo?.hasLogin ? "border-green-200 bg-green-50" : "border-amber-200 bg-amber-50"
        }`}
      >
        <Smartphone
          className={`w-4 h-4 mt-0.5 shrink-0 ${
            loginInfo?.hasLogin ? "text-green-600" : "text-amber-600"
          }`}
        />
          ...
          {loginInfo?.hasLogin ? (
            <>
              ...
              <p className="text-green-700 mt-0.5">
                ...
                <code className="font-mono">{BASE_PATH || ""}{signInPath}</code>{" "}
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            placeholder={t("fieldEmployeeDetail.passwordPlaceholder")}
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {showOnboardingInvite && !loginInfo?.hasLogin && (
          <PillButton
            type="button"
            color="image"
            disabled={credBusy}
