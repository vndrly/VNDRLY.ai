...
export default function PartnerApprovalLogo({ partnerId, partnerName, logoUrl, squareLogoUrl }: { partnerId: number; partnerName: string; logoUrl?: string | null; squareLogoUrl?: string | null }) {
  ...
  return <img src={source} alt={`${partnerName} logo`} className="h-8 w-8 shrink-0 rounded-md border-2 border-[color:var(--brand-primary)] bg-white object-contain" data-testid={`img-partner-row-${partnerId}`} onError={() => setErrored(true)} />;
}
