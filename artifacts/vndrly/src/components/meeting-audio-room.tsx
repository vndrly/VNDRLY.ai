...
export default function MeetingAudioRoom({ occurrenceId }: { occurrenceId: string }) {
  ...
  const report = (cause: unknown) => setError(cause instanceof Error ? cause.message : "Audio operation failed. Please retry.");
  ...
  const peerFor = (userId: number) => {
    ...
    peer.ontrack = (event) => { const stream = event.streams[0] ?? new MediaStream([event.track]); remote.current.set(userId, stream); connectRecordingStream(stream); const audio = audioElements.current.get(userId) ?? new Audio(); audio.autoplay = true; audio.srcObject = stream; audioElements.current.set(userId, audio); void audio.play().catch(() => report(new Error("Audio playback was blocked. Use Resume audio."))); };
    peer.onconnectionstatechange = () => { if (peer.connectionState === "failed") report(new Error("Audio connection failed. Rejoin the call; restrictive networks require the VNDRLY relay.")); };
    peers.current.set(userId, peer); return peer;
  };
  const join = async () => {
    setBusy(true); setError(null);
    try {
      if (!navigator.mediaDevices?.getUserMedia || typeof RTCPeerConnection === "undefined") throw new Error("Use a current browser over HTTPS for internal audio.");
  ...
  return <section className="rounded-lg border p-4"><p className="mb-3 text-sm">VNDRLY internal audio · invited participants only</p><p role="status" className="mb-3 text-sm">{capture ? "Recording and transcription are active." : "Recording and transcription are off."}</p>{error && <p role="alert" className="mb-3 text-sm text-red-700">{error}</p>}{!joined ? <BrandPillButton tone="green" disabled={busy} onClick={() => void join()}>{busy ? "Joining…" : "Join audio"}</BrandPillButton> : <div className="flex flex-wrap gap-2"><BrandPillButton tone={muted ? "brand" : "green"} onClick={toggleMute}>{muted ? "Unmute" : "Mute"}</BrandPillButton><BrandPillButton tone={handRaised ? "amber" : "brand"} onClick={toggleHand}>{handRaised ? "Lower hand" : "Raise hand"}</BrandPillButton><BrandPillButton tone="brand" onClick={() => audioElements.current.forEach(audio => void audio.play().catch(report))}>Resume audio</BrandPillButton>{joined.recordingAllowed && <BrandPillButton tone={consented ? "green" : "brand"} onClick={() => { const next = !consented; if (!next) stopCapture(); void workHubRequest(`/meetings/${occurrenceId}/consent`, { method: "POST", body: JSON.stringify({ policyVersion: joined.policyVersion, response: next ? "accepted" : "declined" }) }).then(() => setConsented(next)).catch(report); }}>{consented ? "Withdraw recording consent" : "Consent to recording"}</BrandPillButton>}{joined.recordingAllowed && joined.participants.some(x => x.userId === joined.userId && x.role === "host") && <BrandPillButton tone={capture ? "red" : "brand"} disabled={busy} onClick={() => { setBusy(true); void (async () => { if (capture) await stopCapture(true); await workHubRequest(`/meetings/${occurrenceId}/recording`, { method: "POST", body: JSON.stringify({ enabled: !capture }) }); setCapture(!capture); })().catch(report).finally(() => setBusy(false)); }}>{capture ? "Stop recording" : "Start recording"}</BrandPillButton>}<BrandPillButton tone="red" onClick={leave}>Leave</BrandPillButton></div>}</section>;
}
