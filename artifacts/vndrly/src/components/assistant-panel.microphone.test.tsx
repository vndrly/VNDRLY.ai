...
it('groups microphone and across-VNDRLY controls under one recognizable settings button', () => {
  ...
  expect(screen.getAllByRole('button', { name: 'Click to Mute V' })).toHaveLength(1);
  expect(within(screen.getByTestId('assistant-brand-controls')).getByRole('button', { name: 'Click to Mute V' })).not.toBeNull();
});
