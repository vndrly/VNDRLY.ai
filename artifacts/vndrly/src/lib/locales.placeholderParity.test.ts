...
// (office web app).
//
// `locales.parity.test.ts` next door confirms every English key has a
// non-empty Spanish counterpart, but it looks only at the *presence* of a
// string — not at what's inside it. That leaves a silent failure mode: an
// English string like "Ticket assigned to {{name}}" can ship with a Spanish
// translation of "Ticket asignado a" (placeholder dropped) or
// "Ticket asignado a {{nombre}}" (placeholder renamed). i18next would render
// the literal text "Ticket asignado a " or "Ticket asignado a {{nombre}}" to
// Spanish-speaking dispatchers at runtime, while every parity assertion still
// passes.
//
...
// to recognise it.
//
// The mobile twin lives at
// `artifacts/vndrly-mobile/lib/locales/placeholderParity.test.ts`.
// ---------------------------------------------------------------------------
...
describe("locale placeholder parity (Task #627, web)", () => {
  it("every {{placeholder}} in en.json appears in the matching es.json value", () => {
    ...
    expect(
      ...
      "Spanish translations whose interpolation placeholders do not match " +
        ...
        "(e.g. {{name}} → {{nombre}}) leaves Spanish-speaking dispatchers " +
        "with a broken sentence at runtime:\n" +
