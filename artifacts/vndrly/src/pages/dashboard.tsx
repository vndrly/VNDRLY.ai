...
import { SafetyTrainingBanner } from "@/components/safety-training-banner";
...
export default function Dashboard() {
  ...
  const analyticsCards = useMemo<DashboardAnalyticsCard[]>(() => {
    ...
    return [
      {
        key: "site-throughput",
        title: "Site Throughput",
        value: `${tickets.length} tracking records`,
        detail: `${siteWorkloadRows.length} active site locations`,
        subdetail: "Drill down to site activity and workload",
        icon: MapPin,
        href: "/site-locations",
      },
      {
        ...
        value: `${byVendor.size} vendors`,
        detail: `${awaitingAcceptance.length} awaiting response`,
        subdetail: `${kickedBack} records require rework`,
        icon: Handshake,
        href: "/vendors",
      },
      {
        key: "workforce-utilization",
        title: "Workforce Utilization",
        value: `${tickets.filter((ticket) => activeStatuses.has(ticket.status)).length} active`,
        detail: "Travel, waiting, and on-site work",
        subdetail: "Open the Site Map for current crews",
        icon: Users,
        href: "/site-map",
      },
      {
        ...
        value: "Credential health",
        detail: "Current, 90-day warning, and expired",
        subdetail: "Review role-required certifications",
        icon: ShieldAlert,
        href: "/field-employees",
      },
      {
        key: "operational-reliability",
        title: "Operational Reliability",
        value: `${staleTickets.length} stale`,
        detail: "Late, stale, or delayed active work",
        subdetail: staleTickets.length > 0 ? `Oldest ${staleOldest}d` : "No stale work",
        icon: TimerReset,
        href: "/tickets",
      },
      {
        ...
        detail: "Approval and payment-cycle health",
        subdetail: apAges.length > 0 ? `Average age ${Math.round(average(apAges))}d` : "No AP backlog",
        icon: Landmark,
      ...
      },
      {
        key: "safety-trends",
        title: "Safety Trends",
        value: siteRisk ? `Risk ${siteRisk.score}` : "No open trend",
        detail: "Events, near misses, and leading indicators",
        subdetail: siteRisk?.siteName,
        icon: AlertTriangle,
        href: "/reports",
      },
      {
  ...
  return (
    ...
    >
      ...
      <div className={cn("grid grid-cols-2 gap-3 md:grid-cols-4", isVendor ? "lg:grid-cols-5" : "lg:grid-cols-7")}>
        {summaryLoading
