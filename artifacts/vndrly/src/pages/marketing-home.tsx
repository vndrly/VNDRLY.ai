...
function PrimaryLink({ children, href }: { children: React.ReactNode; href: string }) {
  return (
    <a href={href} className="inline-flex min-h-11 items-center focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300">
    ...
    <main className="min-h-screen bg-[#f3f5f6] text-slate-950" data-testid="marketing-home">
      ...
      <section className="relative isolate overflow-hidden bg-[#20262b] text-white">
        ...
        <div className="relative mx-auto grid min-h-[660px] max-w-7xl items-center gap-12 px-4 py-20 lg:grid-cols-[1.12fr_.88fr] lg:py-28">
          <div>
            <p className="text-sm font-black uppercase tracking-[.24em] text-amber-300">Verified vendors. Connected operations.</p>
      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-20 lg:grid-cols-2">
        <article id="partners" className="rounded-3xl border bg-white p-7 shadow-sm sm:p-9">
          ...
          <h2 className="mt-2 text-3xl font-black text-amber-700">For partners: know who can do the work—and what is happening now.</h2>
          <BenefitList items={PARTNER_BENEFITS}/>
        </article>
        <article id="vendors" className="rounded-3xl border bg-white p-7 shadow-sm sm:p-9">
          <BriefcaseBusiness className="h-9 w-9 text-amber-700"/>
          <SectionLabel>For vendors</SectionLabel>
          <h2 className="mt-2 text-3xl font-black text-amber-700">For vendors: make proven performance easier to find and easier to repeat.</h2>
        ...
        <div className="mx-auto max-w-7xl px-4 py-20">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {MARKETING_MODULES.map((module, index) => {
              ...
              return <article key={module.title} className="rounded-2xl border border-slate-300 bg-[#f7f8f9] p-6"><Icon className="h-7 w-7 text-amber-700"/><h3 className="mt-4 text-xl font-black text-amber-700">{module.title}</h3><p className="mt-3 text-sm leading-6 text-slate-600">{module.description}</p></article>;
            })}
          </div>
        </div>
      </section>
      ...
      <section aria-label="Secure direct payments" className="relative overflow-hidden bg-gradient-to-br from-cyan-950 via-slate-900 to-slate-950 text-white">
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-4 py-16 md:grid-cols-[auto_1fr_auto]">
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl border border-amber-300/30 bg-amber-300/10"><CreditCard className="h-8 w-8 text-amber-300"/></div>
      ...
      <section className="mx-auto grid max-w-7xl gap-10 px-4 py-20 lg:grid-cols-[1fr_.9fr]">
        <div>
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2">{[[BadgeCheck,"Verified operational signals"],[ShieldCheck,"Role and organization permissions"],[Star,"Reputation built through work"],[Network,"No public account directory"]].map(([Icon,label]) => { const TrustIcon = Icon as typeof BadgeCheck; return <div key={label as string} className="flex items-center gap-3 rounded-xl border bg-white p-4 font-bold text-amber-700"><TrustIcon className="h-5 w-5 text-amber-700"/>{label as string}</div>; })}</div>
        </div>
        <aside className="rounded-3xl border-2 border-amber-500/60 bg-white p-8 shadow-xl">
          <SectionLabel>Ask V — Product Guide</SectionLabel>
          <h2 className="mt-3 text-3xl font-black text-amber-700">Questions before you sign in?</h2>
