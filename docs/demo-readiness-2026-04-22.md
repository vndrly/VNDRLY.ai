...
|---|---|---|---|---|---|
| 1 | Partner creates a ticket via QR scan with GPS | N/A | PENDING-DEVICE | PENDING-DEVICE | Mobile-only flow per task spec. Server contract confirmed: `POST /api/field/tickets` with `{siteLocationId, workTypeId, latitude, longitude, initialState:"pending_arrival"}` returns 201 with a draft ticket and `lifecycle_state="pending_arrival"`. Screen reviewed in `artifacts/vndrly-mobile/app/new-ticket.tsx`. |
...
| F3 | Only 4 of 96 seeded tickets have `lifecycle_state` populated. Tickets created during the demo will display the lifecycle correctly, but historical rows show blank lifecycle badges. | **nice-to-have** | Open — see follow-up #127 (pre-seed in-flight lifecycle tickets). |
