...
Office fallback reloads the active membership and resolves the destination through the same live authority checks before any push/email. An unavailable, unauthorized or disabled recipient/category cancels all existing retryable channel rows in one owner-scoped update, including future-due retries. Channel-specific disablement cancels that channel without blocking the others. A gate-to-office change always cancels stale SMS work. Accepted, uncertain and in-flight audit records are not rewritten. Cancellation is terminal and removes retry scheduling, so old unavailable recipients cannot monopolize the worker's 100-row batch.

The existing notification worker retries only definite retryable failures, up to three attempts with a one-minute-per-attempt delay. Accepted sends and skipped channels are not replayed. Explicit SendGrid HTTP 429 and 503 responses are definite transient rejections and retry; transport timeouts/disconnects, HTTP 408 and unclassified server responses remain ambiguous. Pre-provider badge/count lookup failures remain retryable, including office fallback, and make no provider call. Ambiguous email/SMS failures are recorded as `unknown`; a process interruption after a claim leaves `sending`. These are deliberately not automatically resent because acceptance may already have occurred. Operations can reconcile them against provider records.
...
`DeviceNotRegistered` first persists `cleanup_pending`, then deletes the matching user's exact registration, and only afterward becomes terminal `failed`. A crash or deletion failure leaves durable cleanup work. The worker retries cleanup independently of authorization and exhausted channel retry budgets, without sending; later notifications check the persisted stale-token record before contacting Expo. A different replacement token is never removed. Guarded stale-token and cleanup indexes support these lookups.
